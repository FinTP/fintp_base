...
#ifndef EXTENSIONNEWLINE_H
#define EXTENSIONNEWLINE_H
...
namespace FinTP
{
	class ExportedObject FunctionReplace : public Function
	{
		public:
			...
			#else
				virtual FunctionReplace* clone( MemoryManagerType& theManager ) const;
			#endif
		protected:
			...
			#else
				virtual FunctionReplace* clone() const;
			#endif
		protected:
		...
		private:
			...
			FunctionReplace& operator=( const FunctionReplace& );
			bool operator==( const FunctionReplace& ) const;
	};
}

#endif // EXTENSIONNEWLINE_H
